...
d3.json("data/all-car-path.json", function(error, lines) {
   ...
   let visitChart = new Chart2D(mc1.visitDurationSvg, width, height, {id: 1, margin: margin, timeChart: true});
    ...
    let simulationManager = new SimulationManager(mc1.parkMap);

    mc1.firstDayDuration = new VisitTimeBlock(firstDaySpanChart, mc1.parkMap, null, null, eventHandler, simulationManager);
    mc1.firstDayDuration.render(parsedVisits);

    mc1.visitDuration = new VisitDuration(visitChart, mc1.parkMap, null, null, eventHandler, simulationManager);
